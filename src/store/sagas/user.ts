...
function* fetchUser$(action: FetchUserAction) {
  try {
    ...
    const user : UserTypes = yield call(userApi.findUser, userId);
    yield put({
...
function* fetchFriends$(action: FetchFriendsAction) {
  try {
    ...
    const friends : UserTypes = yield call(friendApi.fecthFriendsRequest, id);
    yield put({
...
function* fetchRoomList$(action: FetchRoomListAction) {
  try {
    const id = action.payload;
    const roomList : UserTypes = yield call(chatApi.fetchRoomList, id);
    yield put({
